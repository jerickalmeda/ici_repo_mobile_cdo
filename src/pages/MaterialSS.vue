<template>
  ...
  -->
  <!-- To be deleted below -->
  <div v-for="(thisMaterials, index) in materialHandouts" :key="thisMaterials.id" :name="index" style="text-align:center;" id="allowfullscreen">
	...
	<!--<carousel :navigation-enabled="true" :paginationEnabled="false" :navigationClickTargetSize="0" :per-page="1" :mouse-drag="true" :touchDrag="true" :adjustableHeight="false" :centerMode="false" id="allowfullscreen">
		<slide v-for="(thisMaterials, index) in materialHandouts" :key="thisMaterials.id" :name="index" style="text-align:center;">
...
export default {
  ...
  created: function() {
    axios
      .get("statics/subjecttype/Specialized/" +  this.$route.params.courseCode + "/" + this.$route.params.subjectCode + "/" + this.$route.params.week.split(" ").join("") + "/" + "handout.json", {
          
      })

      .then(response => {
		  (this.materialHandouts = response.data.handout),
