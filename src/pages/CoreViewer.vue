  ...
  <div class="q-pa-md">
        ...
        <q-tab-panels v-model="tab" active animated class="text-black">
            ...
            <div class="text-h6">Materials</div>
              <q-list padding bordered class="rounded-borders">
                ...
                <q-list dense bordered padding class="rounded-borders">
                  <q-item clickable v-ripple v-for="file of material.file" :key="file.id" :to="{ path: '/materialCore/' + course.type + '/'+ material.subject_code + '/' + file.split('-')[0] + '/' + file}">
                    <q-item-section>
                      {{ file }}
                    </q-item-section>
                  </q-item>
          ...
          <q-tab-panel name="videos">
            <div class="text-h6">Videos</div>
              <q-list padding bordered class="rounded-borders">
                <q-expansion-item v-for="material of course.video" :key="material.id"
                  dense
                  dense-toggle
                  expand-separator
                  group="materialgroup"
                ...
                >
                <q-list dense bordered padding class="rounded-borders">
                  <q-item clickable v-ripple v-for="file of material.file" :key="file.id" :to="{ path: '/video/'+ course.type + '/' + material.subject_code + '/' + file.split('-')[0] + '/' + file + '/video.mp4'}">
                    <q-item-section>
                      {{ file }}
                    </q-item-section>
                  </q-item>
          ...
          <q-tab-panel name="handouts">
            <div class="text-h6">Handouts</div>
              <q-list padding bordered class="rounded-borders">
                ...
                >
                <q-list dense bordered padding class="rounded-borders">
                  <q-item clickable v-ripple v-for="file of material.file" :key="file.id" :to="{ path: '/handoutCore/' + course.type + '/'+ material.subject_code + '/' + file.split('-')[0] + '/' + file}">
                    <q-item-section>
                      {{ file }}
                    </q-item-section>
                  </q-item>
    ...
    <q-footer bordered class="text-primary">
      <q-tabs caps active-color="success" indicator-color="transparent" class="text-white">
        <q-btn :to="{ path: '/Core' }" clickable name="back" label="Back to Core Subjects" ></q-btn>
      </q-tabs>
    </q-footer>
...
export default {
  ...
  created: function() {
    axios
      .get("statics/jsons/Core/" + this.$route.params.subjectCode + ".json", {
      })
